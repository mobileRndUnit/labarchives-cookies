...
public class CookieManagerModule extends ReactContextBaseJavaModule {
    ...
    public void set(ReadableMap options, final Callback callback) throws Exception {
        ...
        cm.setCookie("https://" + domain + "/attachments/inline_image/NDg0Mi41fDM3MjUvMzcyNS9Ob3RlYm9vay80MTMxNDU4OTU0fDEyMjkyLjU=?ep_id=MTMuMHwzNzI1LzEwL0VudHJ5UGFydC83OTEzMTA3MjV8MzMuMA%3D%3D&amp;file_name=1497514439338.png", "laapp=" + value);
        CookieSyncManager.getInstance().sync();

        CookieSyncManager cookieSyncManager = CookieSyncManager.createInstance(getCurrentActivity());
        CookieSyncManager.getInstance().startSync();

        CookieManager cookieManager = CookieManager.getInstance();
        cookieManager.setAcceptCookie(true);
        cookieManager.setCookie(origin, name + "=" + value + ";"
            + OPTIONS_PATH + "=" + path + ";"
